...
def generate_and_send_password_user(sender, instance, **kwargs):
    # Do this only for non-LDAP User objects – standard users have None as username, so we check that.
    if not instance.username and not instance.password and instance.id != settings.ANONYMOUS_USER_ID:
...
def generate_and_send_password_person(sender, instance, **kwargs):
    # Do this only for non-LDAP User objects – standard users have None as username, so we check that.
    if hasattr(instance, 'user') and not instance.user.username and not instance.user.password:
